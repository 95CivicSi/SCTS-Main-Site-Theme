...
if ( is_admin()) :
function kurama_remove_pageattr_box() {
	remove_meta_box( 'pageparentdiv', 'page', 'side' );
	add_meta_box('pageparentdivnew', 'Configure Page', 'kurama_page_attributes_meta_box', 'page', 'side', 'high');
}
add_action( 'admin_menu', 'kurama_remove_pageattr_box' );
endif;
 ...
 * @param object $post
 */
function kurama_page_attributes_meta_box( $post ) {

	wp_nonce_field( basename( __FILE__ ), 'kurama_nonce' );
    $kurama_stored_meta_attr = get_post_meta( $post->ID );
...
<select name="choose-category" id="choose-category">
		<option value="all_c" <?php if ( isset ( $kurama_stored_meta_attr['choose-category'] ) ) selected( $kurama_stored_meta_attr['choose-category'][0], 'all_c' ); ?> > <?php _e('All Categories (Default)'); ?></option>
	<?php foreach  ( $categories as $c ) : ?>
		<option value="<?php echo $c->cat_ID; ?>" <?php if ( isset ( $kurama_stored_meta_attr['choose-category'] ) ) selected( $kurama_stored_meta_attr['choose-category'][0], $c->cat_ID ); ?> > <?php echo $c->name; ?></option>
	<?php endforeach; ?>	
</select>
</label>
...
function kurama_meta_save_attr( $post_id ) {
	// Checks save status
...
add_action('save_post', 'kurama_meta_save_attr');